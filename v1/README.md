...
> Legacy snapshot archived under `v1/`.
>
> The original project files were moved from the repo root into this folder so the next iteration can be built in `../v2/`. Imports, docs, and tests below reflect the original layout and may need path adjustments if you run them from the new repo structure.
